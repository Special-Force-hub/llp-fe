...
import { Box, Collapse, containerClasses } from '@mui/material';
...
const getMenuItems = (setMenuOpenInMobile, isOpen, setMenuOpen, menuArray, role, level, selectedMenu, selectedParentMenu, onSelectMenu) => {
      ...
      return (
        <Box key={item.key}>
          <MenuItem
            ...
            level={level}
            onClick={() => {
              onSelectMenu(item.key, item)
              setMenuOpen(false)
              setMenuOpenInMobile(false);
            }}
          />
            ...
            >
              {item.child.map((childMenu) => (
                <MenuItem
                  ...
                  onClick={() => {
                    onSelectMenu(`${item.key}.${childMenu.key}`, childMenu)
                  }}
                  level={level + 1}
                />
              ))}
...
export const Menu = ({ data, isOpen, setMenuOpen }) => {
  ...
  const setMenuOpenInMobile = () => user.setMenuOpenInMobile(false);

  return (
    <Container isOpen={isOpen}>
      {getMenuItems(setMenuOpenInMobile, isOpen, setMenuOpen, data, role, 0, activeMenu, selectedParentMenu, onSelectMenu)}
    </Container>
  );
};
